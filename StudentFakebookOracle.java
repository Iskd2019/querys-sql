...
public final class StudentFakebookOracle extends FakebookOracle {
    ...
    public FirstNameInfo findNameInfo() throws SQLException {
        try (Statement stmt = oracle.createStatement(FakebookOracleConstants.AllScroll,
            ...
            //We d like to know the longest and shortest first names by length. If there are ties between multiple names,
            //report all tied names in alphabetical order.
            int longName;
            int shortName;
            int commonNameCount;
            //long and short name
            //select and order mame by length

            ResultSet rst = stmt.executeQuery(
                "SELECT DISTINCT LENGTH(First_Name), First_Name FROM " + UsersTable + " " +
                "ORDER BY LENGTH(First_Name) ASC, First_Name");
            //initialize name length    
            rst.next();
            shortName = rst.getInt(1);
            info.addShortName(rst.getString(2));
            //while length match, add it
            while (rst.next() && rst.getInt(1) == shortName) {
                info.addShortName(rst.getString(2));
            }
            ...
            //initialize name length    
            rst.next();
            longName = rst.getInt(1);
            info.addLongName(rst.getString(2));
            //while length match, add it
            while (rst.next() && rst.getInt(1) == longName ) {
                info.addLongName(rst.getString(2));
            }
    ...
    public FakebookArrayList<UserInfo> lonelyUsers() throws SQLException {
        ...
        try (Statement stmt = oracle.createStatement(FakebookOracleConstants.AllScroll,
            ...
            while(rst.next()){
                UserInfo ufo = new UserInfo(rst.getInt(1), rst.getString(2), rst.getString(3));
                results.add(ufo);
            }
    ...
    public FakebookArrayList<UserInfo> liveAwayFromHome() throws SQLException {
        ...
        try (Statement stmt = oracle.createStatement(FakebookOracleConstants.AllScroll,
            ...
            /*
                ...
                results.add(u2);
            */
            //idea: expell users that live in hometown
            ResultSet rst = stmt.executeQuery(
                "SELECT DISTINCT User_ID, First_Name, Last_Name FROM " + UsersTable + " " +
                           "WHERE User_ID NOT IN " +
                           "(SELECT DISTINCT Cc.User_ID" + 
                           " FROM " + CurrentCitiesTable + " Cc, " + HometownCitiesTable + " Hc " +
                           "WHERE Cc.User_ID = Hc.User_ID AND Cc.Current_City_ID = Hc.Hometown_City_ID) " +	
                           "ORDER BY User_ID");
    ...
    public FakebookArrayList<TaggedPhotoInfo> findPhotosWithMostTags(int num) throws SQLException {
        ...
        try (Statement stmt = oracle.createStatement(FakebookOracleConstants.AllScroll,
                FakebookOracleConstants.ReadOnly)) {
            //get qualified top tag photo
            // idea: combine photo ablum to get photo info, then union with phototag to get photo with most tagges with its corresponding info
    ...
    public EventStateInfo findEventStates() throws SQLException {
        try (Statement stmt = oracle.createStatement(FakebookOracleConstants.AllScroll,
            ...
            */
            ...
            ResultSet rst = stmt.executeQuery(
                "SELECT Ct.State_Name, COUNT(*) FROM " + 
                CitiesTable + " Ct, " + EventsTable + " Ev WHERE Ct.City_ID = Ev.Event_City_ID" +
                " GROUP BY Ct.State_Name " + 
                "ORDER BY COUNT(*) DESC");
            ...
            rst.next();
            event_count = rst.getInt(2);
            EventStateInfo info = new EventStateInfo(event_count);
            info.addState(rst.getString(1));



            while(rst.next() && rst.getInt(2) == event_count){
    ...
    public AgeInfo findAgeInfo(long userID) throws SQLException {
        try (Statement stmt = oracle.createStatement(FakebookOracleConstants.AllScroll,
            ...
            */
            ...
            ResultSet rst = stmt.executeQuery(
                "SELECT * FROM (SELECT User_ID, First_Name, Last_Name "+
                "FROM " + UsersTable + " " +
                "WHERE User_ID IN (SELECT FA.User2_ID FROM " + FriendsTable + " FA WHERE FA.User1_ID = " + userID + " " + 
                "UNION SELECT FB.User1_ID FROM " + FriendsTable + " FB WHERE FB.User2_ID = " + userID + ") " +
            ...
            UserInfo youngFriend = new UserInfo(id, fname, lname);
            ...
            UserInfo oldFriend = new UserInfo(id, fname, lname);

            return new AgeInfo(oldFriend,youngFriend);
        ...
        try (Statement stmt = oracle.createStatement(FakebookOracleConstants.AllScroll,
            ...
            */
            ResultSet rst = stmt.executeQuery(
                ...
                "FROM " + UsersTable+ " U1, " + UsersTable+ " U2, " + HometownCitiesTable + " H1," + HometownCitiesTable + " H2, " + FriendsTable + " F "+
                "WHERE U1.User_ID = F.User1_ID AND U2.User_ID = F.User2_ID AND F.User1_ID = H1.User_ID AND F.User2_ID = H2.User_ID "+
                ...
                while(rst.next()){
                    UserInfo uaa = new UserInfo(rst.getInt(1), rst.getString(2), rst.getString(3));
                    UserInfo ubb = new UserInfo(rst.getInt(4), rst.getString(5), rst.getString(6));
                    SiblingInfo sfo = new SiblingInfo(uaa, ubb);
                    results.add(sfo);
                }
